...
void easycomm::parse(int TruAzim, int TruElev) {
  ...
  if (Azimuth != ""){
    ...
    this->ComAzim = this->ComAzim%360;          // keeping values between limits(for trackers with more than 360 deg. rotation)
  }
      
  // if <ELxx> received
  if (Elevation != ""){
    this->ComElev = Elevation.toInt();
    if (this->ComElev>180) { this->ComElev = 0;}
    if (this->ComElev>90) {               //if received more than 90deg. (for trackers with 180deg. elevation)
      this->ComElev = 180-this->ComElev;        //keep below 90deg.
      this->ComAzim = (this->ComAzim+180)%360;  //and rotate the antenna on the back
    }
  }
  ...
  for (int i = 0; i <= ComputerRead.length(); i++) {
    if ((ComputerRead.charAt(i) == 'A')&&(ComputerRead.charAt(i+1) == 'Z')&&(ComputerRead.charAt(i+3) == 'E')&&(ComputerRead.charAt(i+4) == 'L')){
      this->serial_port->println("AZ"+String(TruAzim)+".0 EL"+String(TruElev)+".0");
    }
  }
}
